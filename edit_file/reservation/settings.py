...
# 環境変数を読み込むためにosモジュールをインポート
import os
# パスを扱うためにpathlibモジュールをインポート
from pathlib import Path
# Celeryのスケジュール機能を使用するためにインポート
...
# 環境変数を直接書かず、環境変数で管理することでセキュリティに配慮
SECRET_KEY = os.getenv('SECRET_KEY', 'django-insecure-vz5xgkl$efd1_gyhw#pq^y*kmn65-6uy0ub55o*l=1c#8zep&r')

# 下記、実際は.envファイルを作成して環境変数を設定する

# SECURITY WARNING: don't run with debug turned on in production!
# 目的：デバッグモードを有効にするために使用　開発環境ではTrue、本番環境ではFalse
DEBUG = os.getenv('DEBUG', 'True').lower() == 'true'
# どのドメインからのアクセスを許可するか環境ごとに設定
